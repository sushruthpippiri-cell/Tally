...
| SEC-1.9 | 100/min per client IP unauthenticated, 1,000/min per user; client IP from `X-Forwarded-For` only via a trusted proxy (D-033 #5). Per-email login throttle: 10 failures / 15 min, never a permanent lockout (D-033 #7) | `app/core/middleware.py`, `app/services/auth.py` | **Ceiling:** counters are per process; moving them to shared storage is required task **P16.11**, before running more than one backend replica |
