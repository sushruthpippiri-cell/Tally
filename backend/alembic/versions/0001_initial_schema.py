"""Initial schema: every SRS Section 5 table plus the D-entry additions (Phase 1).

Autogenerated from app.models, then hand-reviewed (2026-09-23). Hand-written parts are at the
end: role seeds, the no-DELETE / append-only / immutable-owner triggers and the audit REVOKE.
tests/models/test_migration.py proves upgrade == models and downgrade leaves nothing behind.
...
def upgrade() -> None:
    ...
    sa.Column('id', sa.BigInteger(), autoincrement=True, nullable=False),
    sa.Column('company_id', sa.UUID(), nullable=False),
    sa.Column('voucher_entry_id', sa.BigInteger(), nullable=False),
    sa.Column('ledger_id', sa.UUID(), nullable=False),
